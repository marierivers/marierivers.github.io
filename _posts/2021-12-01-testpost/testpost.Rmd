...
## Background
The Environmental Quality Index (EQI), developed by the U.S. Environmental Protection Agency (EPA) provides a county level snapshop of environmental conditions throughout the country. EPA first released EQIs for the period 2000-2005 and updated these indexes for 2006-2010. This statistical evaluation focuses on the 2006-2010 EQI. The purpose of the EQI is to use (1) as an indicator of ambient conditions/exposure in environmental health and modeling and (2) as a covariate to adjust for ambient conditions in environmental models (EPA 2020). Previous studies have used the EQI to evaluate relationships between environmental quality and public health outcomes such as preterm birth, cancer incidence, asthma, obesity, infant mortality.
...
<strong>Higher EQI values correspond with worse environmental quality. Lower (more negative) EQI values correspond with better environmental quality</strong>  
...
```{r include=FALSE options(knitr.table.format = "pdf"}
eqi_summary_table <- eqi_pop %>%
  ...
  kable(format = "latex", col.names = c("Population Change", "Minimum", "Maximum", "Mean", "Standard Deviation", "Variance", "Number of Counties"), caption = "EQI summary statistics for counties based on population change between 2006 and 2010", color = 'black') %>%
...
```{r echo=FALSE, fig.height=3, fig.width=}
...
ggsave(here("_posts", "2021-11-13-the-epas-environmental-quality-index-eqi-vs-population-change", "figs", "histogram.png"), plot = histogram_pop_change)
```
**null_hypothesis:**
...
$$SE = \sqrt{\frac{s_1^2}{n_1} + \frac{s^2_2}{n_2}} = `r round(SE, 3)`$$ 
...
p-value, probability of getting a point estimate at least as extreme as calculated if the null hypothesis were true: 
...
eqi_lm_plot_big
ggsave(here("_posts", "2021-11-13-the-epas-environmental-quality-index-eqi-vs-population-change", "figs", "lm.png"), plot = eqi_lm_plot_big)
```
...
point estimate, $\beta_1$ = `r round(beta_1_lm, 3)`
...
Since p-value for the slope coefficient < 0.001, we reject the null hypothesis that EQI has no influence on population change at the 0.01% level. There is a statistically significant relationship between EQI and % population change and the coefficient is significantly different from zero. The 95% confidence interval for the slope coefficient ranges from `r lm_ci_95_lower` to `r lm_ci_95_upper`. This means that there is a 95% chance that this interval includes the true county level rate of change for percent population change for each one unit change in EQI.
...
lm_domain_plot
ggsave(here("_posts", "2021-11-13-the-epas-environmental-quality-index-eqi-vs-population-change", "figs", "lm_domain.png"), plot = lm_domain_plot)
```

While a partly manual method was used above, the table below present coefficients for domain specific models generated with statistical functions in R. The numbers in [brackets] are the 95% confidence intervals for each estimated coefficient.
...
```
The p-value on the slope coefficient was < 0.05 for all domain specific linear models which indicates a statistically significant relationship. Based on the $R^2$ values and slope coefficients, the air and sociodemographic domains account for most of the overall relationship between population change and EQI. The $R^2$ terms represents the variance in percent population change that can be explained by EQI. For the overall EQI value, 5% of the variance in percent population change is explained by environmental conditions. The sociademographic EQI explains 5.8% of the variance in population change while the air EQI explains 5.1%. All domains except land are positively correlated with population change. Since higher EQI values indicate poorer environmental quality, these models show that population change is greatest in counties with worse environmental conditions.

## Conclusions
The identified relationships between population change and environmental quality are noteworthy for their public health and environmental justice implications. Increased population trends in areas with worse environmental conditions could result increased incidences of cancer, asthma, obesity, and infant mortality. While this project did not evaluate economic variables, locations with higher environmental quality could also have higher living costs which drive people to move to more affordable places. If economic factors contribute to population growth in counties with poor environmental quality, then this could negatively affect the health of vulnerable populations and perpetuate social inequalities. Further analysis could evaluate trends in mean household income to determine if there is growing income inequality between counties with better and worse environmental quality. Economic variables and other factors influencing demographic shifts from rural areas cities may be stronger predictors of population change than environmental quality. In the future, EPA plans to create a census tract level EQI. This would facilitate a higher resolution statistical evaluation of the relationship between environmental conditions and population change. 
